...
def train_fn(data_loader, model, criterion, optimizer, device, epoch, scheduler=None):
    ...
    for step, (image_q, image_t, context_images, targets,  width, height, classid, imageid) in enumerate(tk0):

        images_q = list(image_q.to(device) for image_q in image_q)
        images_t = list(image_t.to(device) for image_t in image_t)
        context = list(context_images.to(device) for context_images in context_images)
        ...
        if config.pretrain == 'True':
            output = model(images_q, images_t, context,memory)
            memory = output['feature']
        else:
            output = model(images_q, images_t, context)
